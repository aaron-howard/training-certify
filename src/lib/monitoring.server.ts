...
export function trackRequestMetrics(
    ...
    metrics.increment(`http_requests_total{method="${method}",path="${path}",status="${statusCode}"}`)
    metrics.histogram(`http_request_duration_ms{method="${method}",path="${path}"}`, duration)
}
