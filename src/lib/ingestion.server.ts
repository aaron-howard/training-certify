...
function isRetiredExam(vendorId: string, code: string): boolean {
  if (
    vendorId.toLowerCase() !== 'microsoft' &&
    vendorId.toLowerCase() !== 'msft'
  ) {
    return false
  }
  ...
  if (
    RETIRED_SPECIFIC_CODES.some(
      (retiredCode) => upperCode === retiredCode.toUpperCase(),
    )
  ) {
    return true
  }
...
export async function syncCatalogFromITExams(limitVendors?: number) {
  ...
  const vendorsToProcess = limitVendors
    ? vendors.slice(0, limitVendors)
    : vendors
  ...
  for (const vendor of vendorsToProcess) {
    ...
    for (const exam of exams) {
      ...
      const certId = `${exam.vendorId}-${exam.code.toLowerCase()}`.replace(
        /\s+/g,
        '-',
      )
