...
import { API_ROLE_SETS } from './roles'
import { RateLimitPresets } from './rateLimit.server'
import { uploadCertificationProof } from './certificationProofStorage.server'
import { checkCertificationAuthority } from './certificationAuthority.server'
import { invalidateCache } from './cache.server'

/**
 * POST /api/certifications/proof — multipart proof upload (Vercel Blob + DB record).
 */
export async function handlePostCertificationProof(
  request: Request,
): Promise<Response> {
  return withApiMetrics('POST', '/api/certifications/proof', async () => {
    try {
      ...
      if (
        !(await checkCertificationAuthority(db, session, existingCert.userId))
      ) {
