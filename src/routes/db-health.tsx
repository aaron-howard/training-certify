...
import { testDbConnection } from '../api/db-test'
...
function DbHealthPage() {
    ...
    return (
        <div className="p-8 space-y-4">
            ...
            {isLoading && <div className="text-slate-500">Connecting to database...</div>}
            ...
            {health && (
                <div className={`p-4 rounded-lg border ${health.success ? 'bg-emerald-50 text-emerald-900 border-emerald-200' : 'bg-red-50 text-red-900 border-red-200'}`}>
                    <h2 className="font-bold">{health.success ? 'Connected Successfully' : 'Connection Failed'}</h2>
                    {health.success ? (
            ...
            <div className="mt-8 p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <ul className="list-disc list-inside space-y-1 text-sm text-slate-600">
                    <li>DATABASE_URL: {typeof process !== 'undefined' && process.env.DATABASE_URL ? '✅ Set' : '❌ Missing'}</li>
                </ul>
            </div>
        </div>
    )
