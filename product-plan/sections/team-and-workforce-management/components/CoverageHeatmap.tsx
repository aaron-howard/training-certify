...
export function CoverageHeatmap({
  ...
  const requiredCertIds = new Set(
    certificationRequirements.map((r) => r.certificationId),
  )
  const topCerts = certifications
    .filter((c) => requiredCertIds.has(c.id))
    .slice(0, 8)
  ...
  const getCoverageColor = (percentage: number): string => {
    ...
    if (percentage < 25)
      return 'bg-rose-100 dark:bg-rose-950/50 border-rose-300 dark:border-rose-800'
    if (percentage < 50)
      return 'bg-amber-100 dark:bg-amber-950/50 border-amber-300 dark:border-amber-800'
    if (percentage < 75)
      return 'bg-emerald-100 dark:bg-emerald-950/50 border-emerald-300 dark:border-emerald-800'
    ...
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          No certification requirements defined yet. Set requirements to see
          coverage analysis.
        </p>
      </div>
    )
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 overflow-hidden">
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Team coverage across key certifications. Darker colors indicate higher
          coverage.
        </p>
      </div>
      ...
      <div className="overflow-x-auto -mx-6 px-6">
        <div className="inline-block min-w-full">
          <div
            ...
            style={{
              gridTemplateColumns: `200px repeat(${topCerts.length}, minmax(100px, 1fr))`,
            }}
          >
            {/* Header row */}
