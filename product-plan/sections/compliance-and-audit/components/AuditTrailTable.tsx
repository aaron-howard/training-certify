...
export function AuditTrailTable({ auditLogs, onViewDetails, onFilter, onSearch }: AuditTrailTableProps) {
  ...
  const getActionBadge = (action: string) => {
    const badges = {
      ...
      renewal: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300',
      deletion: 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300',
      verification: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300',
      assignment: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300',
      expiration: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300',
      update: 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300',
    }
    return badges[action as keyof typeof badges] || badges.update
  }

  const actions = ['all', 'upload', 'verification', 'renewal', 'assignment', 'deletion', 'expiration', 'update']
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-lg">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Complete Audit Trail</h2>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm text-slate-600 dark:text-slate-400 mr-2">Filter by action:</span>
          {actions.map(action => (
            <button
              key={action}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800">
              ...
              <th className="text-left py-3 px-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Performed By</th>
              <th className="text-left py-3 px-4 text-sm font-semibold text-slate-700 dark:text-slate-300">Affected Resource</th>
            ...
            {auditLogs.slice(0, 20).map(log => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  <span className={`px-2 py-1 text-xs font-medium rounded capitalize ${getActionBadge(log.action)}`}>
                    {log.action}
                  </span>
                </td>
                <td className="py-3 px-4 text-sm">
                  <div className="font-medium text-slate-900 dark:text-slate-100">{log.performedBy.name}</div>
                  <div className="text-xs text-slate-500">{log.performedBy.role}</div>
                </td>
                <td className="py-3 px-4 text-sm">
                  <div className="text-slate-900 dark:text-slate-100 line-clamp-1">{log.affectedResource.name}</div>
                  <div className="text-xs text-slate-500 capitalize">{log.affectedResource.type.replace(/([A-Z])/g, ' $1').trim()}</div>
                </td>
