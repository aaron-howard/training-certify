...
export function ComplianceMetricsOverview({
  metrics,
  onViewTeamCompliance,
}: ComplianceMetricsOverviewProps) {
  const getTrendIcon = () => {
    if (metrics.trend === 'up')
      return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
          {metrics.complianceByTeam.map((team) => (
            ...
            >
              ...
              <div
                className={`text-2xl font-bold ${getComplianceColor(team.compliancePercentage)}`}
              >
...
function MetricCard({ label, value, icon, trend, color }: MetricCardProps) {
  ...
  return (
    <div
      className={`${colorClasses[color]} border rounded-xl p-5 relative overflow-hidden`}
    >
