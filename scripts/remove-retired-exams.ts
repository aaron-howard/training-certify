...
async function main() {
    ...
    const msCerts = await db.select().from(schema.certifications).where(sql`vendor_id = 'microsoft' OR vendor_id = 'msft'`);

    const toDelete = msCerts.filter(c => {
        const code = c.id.replace('microsoft-', '').replace('msft-', '').toUpperCase();

        const isLegacy = RETIRED_LEGACY_PREFIXES.some(prefix => code.startsWith(prefix));
        const isSpecific = RETIRED_SPECIFIC_CODES.some(retiredCode => code === retiredCode.toUpperCase());
    ...
    for (let i = 0; i < idsToDelete.length; i += BATCH_SIZE) {
        ...
        await db.delete(schema.certifications).where(inArray(schema.certifications.id, batch));
    }
