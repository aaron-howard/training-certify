...
async function verifyNotifications() {
    ...
    const certsToAlert = await db.select()
    ...
    for (const cert of certsToAlert) {
        ...
        if (existingNotif.length === 0) {
            ...
            console.log(`⏭️ Notification already exists for ${cert.userId} / ${cert.certificationName}`);
        }

        // Update status in DB if needed
        if (cert.status !== 'expiring') {
            await db.update(schema.userCertifications)
                .set({ status: 'expiring' })
    ...
    for (const team of teams) {
        ...
        const members = await db.select({ userId: schema.userTeams.userId })
        ...
        for (const req of requirements) {
            ...
            if (memberIds.length > 0) {
                const countResult = await db.select({ value: sql`count(DISTINCT ${schema.userCertifications.userId})` })
                    .from(schema.userCertifications)
                    .where(
                        and(
                            sql`${schema.userCertifications.userId} IN ${memberIds}`,
                            eq(schema.userCertifications.certificationId, req.certificationId),
                            sql`${schema.userCertifications.status} IN ('active', 'expiring-soon')`
                        )
                    );
            ...
            if (actualCount < req.targetCount) {
                console.log(`⚠️ Team "${team.name}" has a gap: ${actualCount}/${req.targetCount} for Cert ID ${req.certificationId}`);

                // Notify Manager if they exist
                if (team.managerId) {
                    const existingTeamNotif = await db.select()
                        .from(schema.notifications)
                        .where(
                            and(
                    ...
                    if (existingTeamNotif.length === 0) {
                        ...
                        console.log(`✅ Created compliance-warning for manager ${team.managerId}`);
                    }
                }
            }
        }
